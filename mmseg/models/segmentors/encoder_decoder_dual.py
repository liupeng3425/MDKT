...
class EncoderDecoderDual(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 init_cfg=None, 
                 mode=None):
        super(EncoderDecoderDual, self).__init__(init_cfg)
    ...
    def extract_feat(self, img, depth, wo_depth=False, **kwargs):
        """Extract features from images."""
        if self.mode == 'attn':
            x = self.backbone(img, depth, mode=self.mode)
        else:
            x = self.backbone(img, depth, wo_depth=wo_depth)
        if self.with_neck:
            x = self.neck(x)
        return x
    ...
    def slide_inference(self, img, img_meta, rescale, **kwargs):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_seg_logit = self.encode_decode(crop_img, img_meta, depth = crop_depth)
    ...
    def simple_test(self, img, img_meta, rescale=True, **kwargs):
        ...
        gt_depth_map = kwargs['gt_depth_map']
        if isinstance(gt_depth_map, list):
            gt_depth_map = gt_depth_map[0]

        seg_logit = self.inference(img, img_meta, rescale, depth=gt_depth_map,**kwargs)
