...
class DAFormerHeadAdvFeature(BaseDecodeHead):
    ...
    def __init__(self, **kwargs):
        ...
        self.used_index = decoder_params.get('used_index', self.in_index)
        if isinstance(embed_dims, int):
            embed_dims = [embed_dims] * len(self.used_index)
        ...
        self.embed_layers = {}
        for i, embed_dim in zip(self.used_index, embed_dims):
            in_channels = self.in_channels[i]
            if i == self.used_index[-1]:
