...
class MixVisionTransformerInjAfter(BaseModule):
    ...
    def __init__(self,
        ...
        self.used_index = kwargs.get('used_index', [])
    ...
    def forward_features(self, x, aux_x, mode=None, wo_depth=False):
        B = x.shape[0]
        outs = []
        features_before = []
        attns = []
        ...
        x = self.norm1(x)
        if mode=='a_distance':
            features_before.append(x)
        # stage 1 injection
        if 0 in self.used_index and not wo_depth:
            ds4 = self.inj_depth_embed['0'](aux_x)
            if mode=='attn':
                f_ds4, attn = self.inj_fusion_layer['0'](x, x2=ds4, mode=mode)
                attns.append(attn)
            else:
                f_ds4 = self.inj_fusion_layer['0'](x, x2=ds4)
            x = x + f_ds4
        ...
        if mode=='a_distance':
            features_before.append(x)
        # stage 2 injection
        if 1 in self.used_index and not wo_depth:
            ds4 = self.inj_depth_embed['1'](aux_x)
            if mode=='attn':
                f_ds4, attn = self.inj_fusion_layer['1'](x, x2= ds4, mode=mode)
                attns.append(attn)
            else:
                f_ds4 = self.inj_fusion_layer['1'](x, x2=ds4)
            x = x + f_ds4
        ...
        if mode=='a_distance':
            features_before.append(x)
        # stage 3 injection
        if 2 in self.used_index and not wo_depth:
            ds4 = self.inj_depth_embed['2'](aux_x)
            if mode=='attn':
                f_ds4, attn = self.inj_fusion_layer['2'](x, x2= ds4, mode=mode)
                attns.append(attn)
            else:
                f_ds4 = self.inj_fusion_layer['2'](x, x2= ds4)
            x = x + f_ds4
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        ...
        if mode=='a_distance':
            features_before.append(x)
        # stage 4 injection
        if 3 in self.used_index and not wo_depth:
            ds4 = self.inj_depth_embed['3'](aux_x)
            if mode=='attn':
                f_ds4, attn = self.inj_fusion_layer['3'](x, x2= ds4, mode=mode)
                attns.append(attn)
            else:
                f_ds4 = self.inj_fusion_layer['3'](x, x2= ds4)
            x = x + f_ds4
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        outs.append(x)
        if mode=='a_distance':
            return outs, features_before
        elif mode=='attn':
            return outs, attns
        else:
            return outs

    def forward(self, x, x_aux, mode=None, wo_depth=False):
        x = self.forward_features(x, x_aux, mode, wo_depth=wo_depth)
        # x = self.head(x)
...
@BACKBONES.register_module()
class mit_b0_inj_after(MixVisionTransformerInjAfter):
...
@BACKBONES.register_module()
class mit_b1_inj_after(MixVisionTransformerInjAfter):
...
@BACKBONES.register_module()
class mit_b2_inj_after(MixVisionTransformerInjAfter):
...
@BACKBONES.register_module()
class mit_b3_inj_after(MixVisionTransformerInjAfter):
...
@BACKBONES.register_module()
class mit_b4_inj_after(MixVisionTransformerInjAfter):
...
@BACKBONES.register_module()
class mit_b5_inj_after(MixVisionTransformerInjAfter):
